...
alpha.15継続シミュレーション：合成11状態を中心に67試行。施設先行経路で目標108秒→78秒の反例を確認。長期重視は生産を増やすが資金を減らす。固定900秒化・予約全面廃止は支持されず。製品コード変更なし。再現方法・数値・限界は[評価報告](rebuild/SIMULATION_ALPHA15.md)。

alpha.15：レビュー指摘を修正。測定順の継続と魔力による不要な無効化の除去、無効予約の解除・一時評価不能の復帰、全購入経路への1.5秒間隔反映、未予約長期目標による共通期間延長を追加。旧診断互換を維持。詳細は[レビュー修正](rebuild/REVIEW_FIXES_ALPHA15.md)。ブラウザ検証なし。
